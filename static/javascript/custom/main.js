'use strict';
var loginModule = angular.module("dbExpApp", ["ui.bootstrap", "ngCookies","ui.router",
                                    "dbExpApp.explorer", "dbExpApp.login", "dbExpApp.operations"]);

loginModule.constant('base_address', "http://localhost:3000/");
...
loginModule.directive('contentEditable',['$compile', 'editting', function($compile, editting) {
    return {
        ...
        link: function(scope, element) {
            ...
            element.parent().on('dblclick', function(e) {
                scope.$apply(function() {
...
loginModule.factory('alertMsg', [function () {
        return {
            'dropColumn': (table, cols) => "Are you sure you want to delete '" + 
			                               table + "." + cols.join() + "' ? ",
			'copyTable': (src, cols, dest) => "Are you sure you want to copy contents of '" + 
                           src +  "." + cols + "' to '" + dest + "'? ",
			'deleteTable': (params) => {
					var names = [], text;
					for (var p of params) {
						names.push(p.values.name[0]);
					}
					text = names.length > 1 ? "tables" : "table";
					return "Are you sure you want to delete " + text + " '" + names.join() + "' ? "
			},
			'closeTables': () => "Are you sure you want to close all tables? ",
        }              
    }
]);
